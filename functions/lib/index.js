...
exports.floraChat = exports.identifyPlant = exports.verifyEmailOTP = exports.sendEmailOTP = void 0;
...
function createTransporter() {
    ...
    return nodemailer.createTransport({
        service: "gmail",
        auth: {
            user: (_a = process.env.EMAIL_USER) !== null && _a !== void 0 ? _a : "",
            pass: (_b = process.env.EMAIL_PASS) !== null && _b !== void 0 ? _b : "",
        },
    });
}
// ─────────────────────────────────────────────────────────────────────────────
...
exports.sendEmailOTP = functions.https.onCall(async (request) => {
    var _a;
    const email = (_a = request.data.email) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
...
exports.verifyEmailOTP = functions.https.onCall(async (request) => {
    var _a, _b;
    const email = (_a = request.data.email) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
    const code = (_b = request.data.code) === null || _b === void 0 ? void 0 : _b.trim();
    if (!email || !code) {
    ...
    if (!doc.exists) {
        throw new functions.https.HttpsError("not-found", "No code was sent to this email. Please request a new one.");
    }
    const data = doc.data();
    ...
    if (data.code !== code) {
        ...
        throw new functions.https.HttpsError("unauthenticated", `Incorrect code. ${left} attempt${left === 1 ? "" : "s"} left.`);
    }
...
// ─────────────────────────────────────────────────────────────────────────────
exports.identifyPlant = functions.https.onCall(async (request) => {
    if (!request.auth) {
...
// ─────────────────────────────────────────────────────────────────────────────
exports.floraChat = functions.https.onRequest(async (req, res) => {
    var _a, e_1, _b, _c;
    var _d, _e;
    ...
    try {
        ...
        try {
            for (var _f = true, _g = __asyncValues(streamingResp.stream), _h; _h = await _g.next(), _a = _h.done, !_a; _f = true) {
                _c = _h.value;
                _f = false;
                const chunk = _c;
                const text = (_e = (_d = chunk.text) === null || _d === void 0 ? void 0 : _d.call(chunk)) !== null && _e !== void 0 ? _e : "";
                if (text) {

...
/// Verdoro feels like she genuinely knows the user's collection.
class VerdoroContextService {
  final FirebaseFirestore _db;
  VerdoroContextService({FirebaseFirestore? db}) : _db = db ?? FirebaseFirestore.instance;
  ...
  Future<String> buildContext(String uid, String conversationId) async {
    if (uid.isEmpty) return 'Context unavailable at this time.';
    // ── Cache check ──────────────────────────────────────────────────────────
    ...
    List<Map<String, dynamic>> activeCases,
      ...
      if (chatMessages.isNotEmpty) {
        final topics = chatMessages
            ...
            .map((m) {
          final role = m['role'] == 'user' ? 'User' : 'Verdoro';
  ...
  /// Verdoro without the overhead of fetching 5 entries per plant.
